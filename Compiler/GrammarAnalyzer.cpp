...
IMItem* popExpStack() {
	if (errorFlag && expStack.empty()) {
		return new IMItem("error", IMItemKind::String, EntryType::STRING);
	}
...
map<symbolType, IMOpType> doWhileConditionMap = {
	{symbolType::LSS, IMOpType::Blt},
	{symbolType::LEQ, IMOpType::Ble},
	{symbolType::GRE, IMOpType::Bgt},
	{symbolType::GEQ, IMOpType::Bge},
	{symbolType::NEQ, IMOpType::Bne},
	{symbolType::EQL, IMOpType::Beq},
};
map<IMOpType, IMOpType> inverseBranchOpMap = {
	{IMOpType::Bge, IMOpType::Blt},
	{IMOpType::Bgt, IMOpType::Ble},
	{IMOpType::Ble, IMOpType::Bgt},
	{IMOpType::Blt, IMOpType::Bge},
	{IMOpType::Beq, IMOpType::Bne},
	{IMOpType::Bne, IMOpType::Beq},
};
map<IMOpType, IMOpType> BranchOpConstMap = {
	{IMOpType::Bge, IMOpType::Blt},
	{IMOpType::Bgt, IMOpType::Ble},
	{IMOpType::Ble, IMOpType::Bgt},
	{IMOpType::Blt, IMOpType::Bge},
	{IMOpType::Bne, IMOpType::Bne},
	{IMOpType::Beq, IMOpType::Beq},
};
...
int GrammarAnalyzer::grammarAnalyze() {
	...
	for (int i = 0; i < out_cnt; i++) 
		outputFile << output[i];
#endif // Grammar_STAGE
	int hasError = errorHandler.output();
#ifdef IM_STAGE
	imCode.output();
#endif // IM_STAGE
	return hasError;
...
int GrammarAnalyzer::variableDefinition() {
	...
	if (!dup) {
		...
		if (imItemKind == IMItemKind::GlbVar) {
			imCode.insertIMCodeEntry(new IMCodeEntry(imOpType, new IMItem(name, imItemKind, entryType, varOffset)));
		} else {
			imCode.insertIMCodeEntry(new IMCodeEntry(imOpType, new IMItem(name, imItemKind, entryType, varOffset, curFuncDefPtr->getName())));
		}
	}
	arrSize = 1;
	while (lexicalTable[sym_p].symbol == COMMA) {
...
int GrammarAnalyzer::returnFunctionDefinition() {
	...
	if (retType == EntryType::INT) {
		imCode.insertIMCodeEntry(new IMCodeEntry(IMOpType::FuncDef, new IMItem(name, IMItemKind::Func, EntryType::INT)));
	} else {
		imCode.insertIMCodeEntry(new IMCodeEntry(IMOpType::FuncDef, new IMItem(name, IMItemKind::Func, EntryType::CHAR)));
	}
	varOffset += 8;//4 for ra, 4 for preSp
	if (!dup) {
		symbolTable.insertFunc(new FuncEntry(name, retType));
		curFuncDefPtr = symbolTable.getFuncPtr(name);
	}
	if (!parameterList()) {
		popReg();
		return 0;
...
int GrammarAnalyzer::nonReturnFunctionDefinition() {
	...
	lexicalOutput();
	IMItem* curFunc = new IMItem(name, IMItemKind::Func, EntryType::VOID);
	imCode.insertIMCodeEntry(new IMCodeEntry(IMOpType::FuncDef, curFunc));
	varOffset += 8;//4 for ra, 4 for preSp
	if (!dup) {
		symbolTable.insertFunc(new FuncEntry(name, retType));
		curFuncDefPtr = symbolTable.getFuncPtr(name);
	...
	lexicalOutput();
	imCode.insertIMCodeEntry(new IMCodeEntry(IMOpType::NonRetFuncReturn, curFunc));
...
int GrammarAnalyzer::parameterList() {
	...
	name = lexicalTable[sym_p].token;
	varOffset += 4;
	paraItems.push_back(new IMItem(name, IMItemKind::LcVar, entryType, varOffset, curFuncDefPtr->getName()));
	VarEntry* varEntry = new VarEntry(name, entryType, 1, varOffset);
	curFuncDefPtr->insertFuncParas(varEntry);
	symbolTable.insertIdenfr((Entry*)varEntry);
	...
	for (int i = 0; i < paraItems.size(); i++) {
		imCode.insertIMCodeEntry(new IMCodeEntry(IMOpType::ParaInFuncDef, paraItems[i]));
		if (i <= 2) {
			curFuncDefPtr->insertAVar2Reg(pair<IMItem, string>(*paraItems[i], "$a" + to_string(i + 1)));
		}
	}
	output[out_p++] = "<参数表>\n";
	return 1;
...
int GrammarAnalyzer::mainFunction() {
	...
	lexicalOutput();
	symbolTable.insertFunc(new FuncEntry("main", EntryType::VOID));
	curFuncDefPtr = symbolTable.getFuncPtr("main");
	if (lexicalTable[sym_p].symbol != LPARENT) {
		popReg();
		return 0;
	}
	...
	lexicalOutput();
	imCode.insertIMCodeEntry(new IMCodeEntry(IMOpType::FuncDef, new IMItem("main", IMItemKind::Func, EntryType::VOID)));
	varOffset += 8;//4 for ra, 4 for preSp
	if (!compoundStatement()) {
		popReg();
		return 0;
	...
	lexicalOutput();
	imCode.insertIMCodeEntry(new IMCodeEntry(IMOpType::NonRetFuncReturn, new IMItem("main", IMItemKind::Func, EntryType::VOID)));
	output[out_p++] = "<主函数>\n";
	return 1;
}
...
void GrammarAnalyzer::genExpIMCode(IMOpType imOpType) {
	...
	if (item2->getKind() == IMItemKind::Const && item1->getKind() == IMItemKind::Const) {
		...
		// 含有变量
		if (item1->getKind() == IMItemKind::TempVar || item2->getKind() == IMItemKind::TempVar) {
			if (item1->getKind() == IMItemKind::TempVar && item2->getKind() == IMItemKind::TempVar) {
				int temp1 = stoi(item1->getName().substr(1));
				int temp2 = stoi(item2->getName().substr(1));
				if (temp1 > temp2) {
					item3 = item1;
				} else {
					item3 = item2;
				}
			} else if (item1->getKind() == IMItemKind::TempVar) {
				item3 = item1;
			} else {
				item3 = item2;
			}
		...
		if (imOpType == IMOpType::AssignArray && item1->getType() == EntryType::CHAR) {
			entryType = EntryType::CHAR;
		}
		item3->setType(entryType);
		/*
		varOffset += 4;
		item3 = new IMItem(genTempVarName(), IMItemKind::TempVar, entryType, varOffset);*/
...
int GrammarAnalyzer::factor() {
	...
	if (lexicalTable[sym_p].symbol == IDENFR) {
		...
		if (!undef) {
			IMItem* item = symbolTable.findIMItem(name, curFuncDefPtr);
			pushExpStack(item);
		}
		lexicalOutput();
		if (lexicalTable[sym_p].symbol == LBRACK) {
...
int GrammarAnalyzer::assignStatement() {
	...
	if (!undef && !conchange) {
		IMItem* imItem = symbolTable.findIMItem(name, curFuncDefPtr);
		pushExpStack(imItem);
	}
	lexicalOutput();
	if (lexicalTable[sym_p].symbol == LBRACK) {
...
//＜条件＞    ::=  ＜表达式＞[＜关系运算符＞＜表达式＞]
int GrammarAnalyzer::condition(bool isDoWhile) {
	...
	if (lexicalTable[sym_p].symbol == LSS || lexicalTable[sym_p].symbol == LEQ ||
		...
		if (!isDoWhile) {
			...
			branchType = doWhileConditionMap.find(lexicalTable[sym_p].symbol)->second; //IMCode: branchType
		}
...
int GrammarAnalyzer::loopStatement() {
	...
	//do＜语句＞while '('＜条件＞')' 
		...
		IMItem* item2 = popExpStack();
		IMItem* item1 = popExpStack();
		imCode.insertIMCodeEntry(new IMCodeEntry(branchType, item1, item2, new IMItem(dowhile_begin, IMItemKind::String, EntryType::STRING)));
	...
	//for'('＜标识符＞＝＜表达式＞; ＜条件＞; ＜标识符＞＝＜标识符＞(+| -)＜步长＞')'＜语句＞
		...
		if (!undef1 && !conchange1) {
			item1 = symbolTable.findIMItem(name1, curFuncDefPtr);
			//pushExpStack(item1);
		} //<标识符1>
		lexicalOutput();
		if (lexicalTable[sym_p].symbol != ASSIGN) {
		...
		if (!undef2) {
			item2 = symbolTable.findIMItem(name2, curFuncDefPtr);
			//pushExpStack(item2);
		} //<标识符1>
		lexicalOutput();
		if (lexicalTable[sym_p].symbol != PLUS && lexicalTable[sym_p].symbol != MINU) {
		...
		IMItem* condition_item2 = popExpStack();
		IMItem* condition_item1 = popExpStack();
		IMOpType thisBranchType = branchType;
		imCode.insertIMCodeEntry(new IMCodeEntry(thisBranchType, condition_item1, condition_item2, new IMItem(if_end, IMItemKind::String, EntryType::STRING)));
		if (lexicalTable[sym_p].symbol != SEMICN) {
			errorHandler.semicolon(lexicalTable[sym_p].lineNo); //sym_p 不-1
			//popReg();
			//return 0;
		...
		//string dowhile_end = "dowhile_" + dowhileNum + "_end";
		IMOpType for_dowhileBranchType = inverseBranchOpMap[thisBranchType];
		imCode.insertIMCodeEntry(new IMCodeEntry(for_dowhileBranchType, condition_item1, condition_item2, new IMItem(dowhile_begin, IMItemKind::String, EntryType::STRING)));
		//imCode.insertIMCodeEntry(new IMCodeEntry(IMOpType::Jump, new IMItem(dowhile_begin, IMItemKind::String, EntryType::STRING)));
		//imCode.insertIMCodeEntry(new IMCodeEntry(IMOpType::Label, new IMItem(dowhile_end, IMItemKind::String, EntryType::STRING)));
		imCode.insertIMCodeEntry(new IMCodeEntry(IMOpType::Label, new IMItem(if_end, IMItemKind::String, EntryType::STRING)));
		output[out_p++] = "<循环语句>\n";
		return 1;
...
IMItem* GrammarAnalyzer::returnFunctionCall() {
	vector<VarEntry*> *valueParas = new vector<VarEntry*>();
	pushReg();
	string name;
	if (lexicalTable[sym_p].symbol != IDENFR) {
		popReg();
	...
	if (!valueParameterList(name, valueParas)) {
		popReg();
		return 0;
	}
	if (lexicalTable[sym_p].symbol != RPARENT) {
	...
	if (!undef) {
		errorHandler.paraUnmatch(name, lexicalTable[sym_p].lineNo, valueParas);
	}
	//vector<VarEntry*>().swap(*valueParas);
	//symbolTable.clrTempParas();

	imCode.insertIMCodeEntry(new IMCodeEntry(IMOpType::FuncCall, 
		new IMItem("presentSize", IMItemKind::Const, EntryType::INT, varOffset), //const but whose name is not "1"
		new IMItem(name, IMItemKind::String, EntryType::STRING)));

	varOffset += 4;
	EntryType funcRetType = symbolTable.findFuncRetType(name);
	// 函数返回值是LcVar，而不是TempVar
	IMItem* funcRetItem = new IMItem("FunRet_" + genTempVarName(), IMItemKind::LcVar, funcRetType, varOffset, curFuncDefPtr->getName());
...
//＜值参数表＞   :: = ＜表达式＞{ ,＜表达式＞ }｜＜空＞
int GrammarAnalyzer::valueParameterList(string calledFuncName, vector<VarEntry*>* valueParas) {
	pushReg();
	EntryType entryType;
	string name = "hhhhhh";
	int expRet;
	...
	/*
	paraItem = popExpStack();
	paraItems.push_back(paraItem);
	valueParas->push_back(new VarEntry(paraItem->getName(), paraItem->getType(), 1, 0));
	//symbolTable.insertTempParas(new VarEntry(paraItem->getName(), paraItem->getType(), 1, 0), 0); //insertTempParas type=0
	*/
	
	entryType = (expRet == 1) ? INT : CHAR;
	valueParas->push_back(new VarEntry(name, entryType, 1, 0));
	...
	for (int i = 0; i < paraCount; i++) {
		...
		IMItem* item2;
		if (i <= 2) {
			item2 = new IMItem("$a"+to_string(i+1), IMItemKind::ValueParaReg, paraItem->getType());
		} else {
			item2 = new IMItem("pos", IMItemKind::LcVar, paraItem->getType(), varOffset + 12 + i * 4);
		}
		imCode.insertIMCodeEntry(new IMCodeEntry(IMOpType::ParaInFuncCall, paraItem, 
			item2, new IMItem(calledFuncName, IMItemKind::Func, EntryType::STRING)));
		paraItems.pop_back();
	}
	output[out_p++] = "<值参数表>\n";
	return 1;
...
int GrammarAnalyzer::readStatement() {
	...
	if (!undef && !conchange) {
		...
		imCode.insertIMCodeEntry(new IMCodeEntry(IMOpType::Scanf, new IMItem(name, imItemKind, entryType, offset, curFuncDefPtr->getName())));
	}
	lexicalOutput();
...
int GrammarAnalyzer::returnStatement() {
	...
	lexicalOutput();
	IMItem* curFunc = new IMItem(curFuncDefPtr->getName(), IMItemKind::Func, curFuncDefPtr->getType());
	if (lexicalTable[sym_p].symbol == LPARENT) {
		lexicalOutput();
		...
		imCode.insertIMCodeEntry(new IMCodeEntry(IMOpType::RetFuncReturn, curFunc, retItem));
	} else {
		imCode.insertIMCodeEntry(new IMCodeEntry(IMOpType::NonRetFuncReturn, curFunc));
	}
